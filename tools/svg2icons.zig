...
//! never an SVG parser. Each icon in the generated module is its own lazy
//! `pub const` (point tables are tiny — well under a KB each), so an app pays
//! only for the icons it names (#346).
//!
...
fn emit(a: std.mem.Allocator, io: std.Io, out_path: []const u8, icons: []const Icon) !void {
    ...
    try w.writeAll(
        ...
        \\//! Regenerate with `zig build gen-lucide`. Source SVGs and the type
        \\//! definitions (geometry.IconData) are shared across generated sets.
        \\//!
        \\//! Each icon is a `pub const` (e.g. `@import("lucide.zig").house`), so an
        \\//! app compiles in only the icons it names — Zig drops unreferenced decls.
        \\//! `get(Name)` switches over the whole set; calling it pulls every icon in,
        \\//! so reach for it only when you genuinely need the full library (the docs
        \\//! gallery does). It stays lazy: an app that never calls it pays nothing.
        \\const geometry = @import("geometry.zig");
        \\const IconData = geometry.IconData;
        \\
        \\
    );
    ...
    // Per-icon constants. Public + individually named so callers can cherry-pick
    // a single icon without dragging in the rest (the identifier is quoted so
    // names that collide with keywords — e.g. `type` — stay legal).
    for (icons) |ic| {
        try w.print("pub const @\"{s}\" = IconData{{ .stroke_width = {d}, .stroke = {s}, .subpaths = &.{{\n", .{
    ...
    // Dispatch over the whole set (pulls every icon in — see the module header).
    try w.writeAll("pub fn get(n: Name) IconData {\n    return switch (n) {\n");
    for (icons) |ic| try w.print("        .{s} => @\"{s}\",\n", .{ ic.name, ic.name });
    try w.writeAll("    };\n}\n");
