...
pub fn main(init: std.process.Init) !void {
    ...
    var backend_ok = true;
    {
        ...
        // Rounded clip (#117): content masked to a rounded rect, vs raster.
        {
        ...
        // Filled path (#120): concave star via even-odd, vs raster.
        {
            const scene: zooee.fixtures.Scene = .{ .name = "path", .width = 10, .height = 10, .draw = zooee.fixtures.drawPath };
            try zooee.fixtures.run(scene, db.interface(), 8);
            const dpx = try db.readPixels();
            var ras = zooee.backends.raster.RasterBackend.init(gpa);
            defer ras.deinit();
        ...
        // Inset box shadow (#119): a pressed well, shadow on inner edges.
        {
            const scene: zooee.fixtures.Scene = .{ .name = "inset_shadow", .width = 11, .height = 10, .draw = zooee.fixtures.drawInsetShadow };
            try zooee.fixtures.run(scene, db.interface(), 8);
            const dpx = try db.readPixels();
            var ras = zooee.backends.raster.RasterBackend.init(gpa);
            defer ras.deinit();
        ...
        // Rounded-corner box shadow (#119 follow-up): Wallace integral, vs raster.
        {
            const scene: zooee.fixtures.Scene = .{ .name = "rounded_shadow", .width = 12, .height = 11, .draw = zooee.fixtures.drawRoundedShadow };
            try zooee.fixtures.run(scene, db.interface(), 8);
            const dpx = try db.readPixels();
            var ras = zooee.backends.raster.RasterBackend.init(gpa);
            defer ras.deinit();
        ...
        {
            const scene: zooee.fixtures.Scene = .{ .name = "image_fit", .width = 15, .height = 8, .draw = zooee.fixtures.drawImageFit };
            try zooee.fixtures.run(scene, db.interface(), 8);
            const dpx = try db.readPixels();
            var ras = zooee.backends.raster.RasterBackend.init(gpa);
            defer ras.deinit();
        ...
        {
            const scene: zooee.fixtures.Scene = .{ .name = "nine_slice", .width = 14, .height = 7, .draw = zooee.fixtures.drawNineSlice };
            try zooee.fixtures.run(scene, db.interface(), 8);
            const dpx = try db.readPixels();
            var ras = zooee.backends.raster.RasterBackend.init(gpa);
            defer ras.deinit();
        ...
        {
            const scene: zooee.fixtures.Scene = .{ .name = "scroll", .width = 12, .height = 8, .draw = zooee.fixtures.drawScroll };
            try zooee.fixtures.run(scene, db.interface(), 8);
            const dpx = try db.readPixels();
            var ras = zooee.backends.raster.RasterBackend.init(gpa);
            defer ras.deinit();
        ...
        {
            const scene: zooee.fixtures.Scene = .{ .name = "text_layout", .width = 24, .height = 12, .draw = zooee.fixtures.drawTextLayout };
            try zooee.fixtures.run(scene, db.interface(), 8);
            const dpx = try db.readPixels();
            var ras = zooee.backends.raster.RasterBackend.init(gpa);
            defer ras.deinit();
        ...
        {
            const scene: zooee.fixtures.Scene = .{ .name = "text_decoration", .width = 18, .height = 9, .draw = zooee.fixtures.drawTextDecoration };
            try zooee.fixtures.run(scene, db.interface(), 8);
            const dpx = try db.readPixels();
            var ras = zooee.backends.raster.RasterBackend.init(gpa);
            defer ras.deinit();
        ...
        {
            const scene: zooee.fixtures.Scene = .{ .name = "layout_effects", .width = 14, .height = 15, .draw = zooee.fixtures.drawLayoutEffects };
            try zooee.fixtures.run(scene, db.interface(), 8);
            const dpx = try db.readPixels();
            var ras = zooee.backends.raster.RasterBackend.init(gpa);
            defer ras.deinit();
